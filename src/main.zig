...
pub fn main(init: std.process.Init) !void {
    ...
    main_loop: while (!vk_ctx.window.shouldClose()) {
        ...
        if (glfw_cb.takeResizeInfo()) |extent| {
            ...
            helper.io.sleep(.fromMilliseconds(100), .awake) catch unreachable;
        }
        ...
        if (!print_state_failed and last_print_state_timestamp.durationTo(main_loop_timestamp).toMilliseconds() >= 500) {
            if (printStateTick(controller, simulation_start_timestamp.durationTo(simulation_timestamp).nanoseconds, timer)) {
                last_print_state_timestamp = main_loop_timestamp;
            } else |err| {
...
fn printStateTick(controller: Controller, time: i96, timer: anytype) !void {
