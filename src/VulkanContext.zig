...
const UniformOffsets = [in_flight_count]u64;
...
fn _createSets(device: vk.DeviceProxy, pool: vk.DescriptorPool, layouts: SetLayouts) !Sets {
    ...
    var set_layouts: [in_flight_count][set_layout.layout_count]vk.DescriptorSetLayout = undefined;
...
fn _updateUniformDesriptor(device: vk.DeviceProxy, buffers: UniformBuffers, sets: Sets) void {
    var infos: [in_flight_count]vk.DescriptorBufferInfo = undefined;
...
pub const FrameResouces = struct {
    ...
    pub fn beginSettingUniforms(self: *FrameResouces) !*shader_layout.Uniform {
        const map = try self.device.mapMemory(self.uniform_memory, self.uniform_range[0], self.uniform_range[1] - self.uniform_range[0], .{});
    ...
    pub fn drawFrame(self: FrameResouces, info: DrawFrameInfo) !void {
        _updateSurfaceDescriptor(self.device, self.swapchain_view, self.sets[2]);
...
pub fn acquireFrame(self: *VulkanContext) !?FrameResouces {
    ...
    const resources: FrameResouces = .{
        ...
        .uniform_range = .{ uniform_start, if (self.next_frame == 0) self.uniform_offsets[1] else self.uniform_size },
